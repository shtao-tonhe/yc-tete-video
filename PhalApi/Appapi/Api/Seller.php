...
class Api_Seller extends PhalApi_Api {
    ...
    public function getGoodsOrderInfo(){
        ...
        if($order_info['status']==1){ //订单待发货
        	$order_info['status_name']=$Think.\lang('TO_BE_SHIPPED');
        ...
        if($refund_status==1){ //已经完成
        	...
        	if($is_platform_interpose==1){ //平台介入
        		...
        		if($platform_result==1){ //平台同意
        			$rs['code']=1001;
		            $rs['msg']=$Think.\lang('PLATFORM_AGREED_REFUND');
		            return $rs;

        		}elseif($is_platform_interpose==-1){ //平台拒绝
        			$rs['code']=1001;
		            $rs['msg']=$Think.\lang('PLATFORM_REFUSED_REFUND');
		            return $rs;
        		}
    ...
    public function setPlatformGoods(){
		...
		if($res==1002){
			$rs['code']=1004;
			$rs['msg']=$Think.\lang('FAILED_TO_ADD_PRODUCT');
			return $rs;
		}
        ...
        $status=$res['status'];
        if($status){
            $rs['msg']=$Think.\lang('ADD_SUCCESS');
        }else{
            $rs['msg']=$Think.\lang('CANCELLATION_SUCCEEDED');
        }
        $rs['info'][0]=$res;
