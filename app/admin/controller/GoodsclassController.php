...
class GoodsclassController extends AdminbaseController {
    ...
    */
    function addPost(){
        ...
        if(mb_strlen($gc_name)>30){
            $this->error($Think.\lang('CLASSIFCATION_NAME_CHARACTERS'));
        }

        //判断分类名称是否重复
        $isexist=Db::name("shop_goods_class")->where("gc_name='{$gc_name}'")->find();
        if($isexist){
            $this->error($Think.lang('CATEGORY_NAME_ALREADY_EXISTS'));
        }

        if($gc_sort==''){
            $this->error($Think.\lang('FILL_IN_CLASSIFICATION_NUM'));
        }

        if($gc_sort<0){
            $this->error($Think.\lang('SORT_NUM_MUST_INTEGER'));
        }

        if($gc_sort>999999){
            $this->error($Think.\lang('SORTING_NUM_AN_INTEGER'));
        }
        ...
        if(!$result){
             $this->error($Think.\lang('FAILED_TO_ADD_CLASSIFICATION'));
        }
        ...
        $this->success($Think.\lang('CATEGORY_ADDED_SUCCESSFULLY'));
    }
    ...
    */
    function editPost(){
            ...
            switch ($parent_grade) {
                ...
                case 1: //上级为一级分类【当前分类设置为二级分类】
                    ...
                    if($gc_grade==1){ //当前分类为一级
                        ...
                        if($three_info){
                           $this->error($Think.\lang('CLASSIFCATION_EXCEEDS_TWO_LEVERLS'));
                        }
                ...
                case 2://上级为二级分类【当前分类设置为三级分类】
                    ...
                    if($gc_grade==1 || $gc_grade==2){ //当前分类为一级或二级
                        ...
                        if($info){
                            $this->error($Think.\lang('CLASSIFCATIONS_UNDER_HAVA_CLASSIFCATION'));
                        }
        ...
        $this->success($Think.\lang('COMMODITY_CLASSIFICATION_UPDATE_SUCCESSFULLY'));
    ...
    */
    function listOrder(){
        ...
        $action=$Think.\lang('UPDATE_PRODUCT_CLASSIFCATION_SORT');
        setAdminLog($action);
    ...
    */
    ...
    function del(){
        ...
        if($info){
            $this->error($Think.\lang('DELETE_CHILD_CLASSIFICATION'));
        }
