...
class PluginController extends AdminBaseController
{
    ...
    public function toggle()
    {
        ...
        if (empty($pluginModel)) {
            $this->error($Think.\lang('PLUG_NOT_EXIST'));
        }

        $status         = 1;
        $successMessage = $Think.\lang('ENABLED_SUCCESSFULLY');

        if ($this->request->param('disable')) {
            $status         = 0;
            $successMessage = $Think.\lang('DISABLED_SUCCESSFULLY');
        }
    ...
    public function setting()
    {
        ...
        if (empty($plugin)) {
            $this->error($Think.\lang('PLUG_NOT_INSTALLED'));
        }
...
//        }
    ...
    public function install()
    {
        ...
        if ($pluginCount > 0) {
            $this->error($Think.\lang('PLUG_INSTALLED'));
        }

        $plugin = new $class;
        $info   = $plugin->info;
        if (!$info || !$plugin->checkInfo()) {//检测信息的正确性
            $this->error($Think.\lang('PLUG_INFORMATION_MISSING'));
        }

        $installSuccess = $plugin->install();
        if (!$installSuccess) {
            $this->error($Think.\lang('PLUG_PRE_INSTALLATION_FAILED'));
        }
        ...
        $this->success($Think.\lang('INSTALLATION_SUCCEEDED'));
    }
...
//                                    array_push($newMenus, "$app/$controllerName/$action 已更新");
    ...
    public function uninstall()
    {
        ...
        if ($result !== true) {
            $this->error($Think.\lang('UNINSTALL_FAILED'));
        }

        Cache::clear('init_hook_plugins');
        Cache::clear('admin_menus');// 删除后台菜单缓存

        $this->success($Think.\lang('UNINSTALL_SUCCEEDED'));
    }
