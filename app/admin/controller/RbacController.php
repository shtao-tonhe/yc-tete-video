...
class RbacController extends AdminBaseController
{
    ...
    public function roleAddPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if ($result) {
                    $this->success($Think.\lang('ROLE_ADD_SUCCESSFULLY'), url("rbac/index"));
                } else {
                    $this->error($Think.\lang('FAILED_TO_ADD_ROLE'));
                }
    ...
    public function roleEdit()
    {
        ...
        if ($id == 1) {
            $this->error($Think.\lang('ADMINISTRATOR_ROLE_NOT_MODIFIED'));
        }
        $data = Db::name('role')->where("id", $id)->find();
        if (!$data) {
            $this->error($Think.\lang('ROLE_NOT_EXIST'));
        }
        ...
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if (Db::name('role')->update($data) !== false) {
                    $this->success($Think.\lang('EDIT_SUCCESS'), url('rbac/index'));
                } else {
    ...
    public function roleDelete()
    {
        $id = $this->request->param("id", 0, 'intval');
        if ($id == 1) {
            $this->error($Think.\lang('ADMINISTRATOR_ROLE_NOT_DELETED'));
        }
        $count = Db::name('RoleUser')->where('role_id', $id)->count();
        if ($count > 0) {
            $this->error($Think.\lang('ALREADY_USERS_IN_ROLE'));
        } else {
            $status = Db::name('role')->delete($id);
            if (!empty($status)) {
                $this->success($Think.\lang('DELETE_SUCCESS'), url('rbac/index'));
            } else {
    ...
    public function authorize()
    {
        ...
        if (empty($roleId)) {
            $this->error($Think.\lang('PARAMENTER_ERROT'));
        }
    ...
    public function authorizePost()
    {
        if ($this->request->isPost()) {
            ...
            if (!$roleId) {
                $this->error($Think.\lang('THE_ROLE_REQIRING_NOT_EXIST'));
            }
            if (is_array($this->request->param('menuId/a')) && count($this->request->param('menuId/a')) > 0) {
                ...
                $this->success($Think.\lang('AUTHORIZATION_SUCCEEDED'));
            } else {
                //当没有数据时，清除当前角色授权
                Db::name("authAccess")->where("role_id", $roleId)->delete();
                $this->error($Think.\lang('NOT_DATA_IS_RECEIVER'));
            }
        }
    }
